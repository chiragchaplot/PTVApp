...
//
//   let stopsAroundLocation = try? newJSONDecoder().decode(StopsAroundLocation.self, from: jsonData)

import Foundation

// MARK: - StopsAroundLocation
struct StopsAroundLocation: Codable {
    let stops: [Stop]
...
struct Stop: Codable {
    ...
    let stopName: String
    let stopID, routeType: Int
    let routes: [Route]
    let stopLatitude, stopLongitude: Double
    let stopLandmark: String
...
struct Route: Codable {
    let routeType, routeID: Int
    let routeName, routeNumber, routeGtfsID: String
    let geopath: [JSONAny]
...
enum StopSuburb: String, Codable {
    case footscray = "Footscray"
    case seddon = "Seddon"
}
